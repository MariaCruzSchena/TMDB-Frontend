...
const Login = ({ interaction }) => {
  ...
  const postUserHandler = async (e) => {
    ...
    try {
      //Login request
      const response = await axios.post(postUserRoute, postInfo, { withCredentials: true,  credentials: 'include'});
      dispatch(setUser(response.data));
      localStorage.setItem("user", JSON.stringify(response.data));

      //Get users favorites
      const favoritesResponse = await axios.get(
        `/api/user/${response.data.id}/favorites`, { withCredentials: true,  credentials: 'include'}
      );
      localStorage.setItem("favorites", JSON.stringify(favoritesResponse.data));
      dispatch(setFavorites(favoritesResponse.data));

      //Get users in db
      const DBUsersResponse = await axios.get("/api/user", { withCredentials: true,  credentials: 'include'});
